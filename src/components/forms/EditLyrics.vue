    ...
    <form class="flex flex-col sm:flex-row sm:justify-between flex-wrap gap-4" @submit.prevent="handleSubmit">
        ...
        <div class="flex w-full gap-4 flex-wrap xs:flex-nowrap">
            <div class="flex items-center gap-2 flex-wrap">
                <ButtonForm
                    ...
                    @click="
                        handleSearchSyncedLyrics(
                            generateLyricsSearchQuery(video.metadata, form.fields.track, form.fields.album, form.fields.artist),
                        )
                    "
                >
                ...
                <ButtonForm
                    :class="'h-8 !rounded-full hover:!ring-violet-400 hover:dark:!ring-violet-700 hover:ring-[0.125rem]'"
                    :disabled="isLoadingLyrics"
                    @click="resetLyrics"
                >
            ...
            <p class="text-sm ml-auto h-8 text-nowrap items-center flex min-w-fit" v-show="hasSearchedForLyrics">
        ...
        <div class="flex flex-col gap-2 w-full" v-if="isLoadingLyrics || hasSearchedForLyrics || searchResults.length !== 0">
            <div class="flex gap-2 flex-col w-full">
                <LrcLibCard
                    ...
                    :key="result.id"
                    :data="result"
                    @preview="handlePreviewLyrics(result)"
                    @select="handleSelectLyrics(result)"
                />
                <div
        ...
        <p
            class="text-sm text-rose-600 dark:text-rose-400 text-center w-full"
            v-if="form.fields.lyrics !== video.metadata?.lyrics && video.metadata?.lyrics"
        >
        ...
        <p
            class="text-sm text-rose-600 dark:text-rose-400 text-center w-full"
            v-if="form.fields.artist !== video.metadata?.artist && video.metadata?.artist"
        >
        ...
        <p
            class="text-sm text-rose-600 dark:text-rose-400 text-center w-full"
            v-if="form.fields.album !== video.metadata?.album && video.metadata?.album"
        >
