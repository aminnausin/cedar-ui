...
import InputMultiChip from '@/components/core/InputMultiChip.vue';
...
const handleSetTags = (newTags: VideoTagResource[]) => {
    ...
    form.fields['deleted_tags'] = existingVideoTags
        .filter((videoTag) => !newTags.some((tag) => tag.id === videoTag.id))
        .map((videoTag) => videoTag.video_tag_id);
};
